...
class MoveCommand(sx:Int, sy:Int, dx:Int, dy:Int, controller:Controller) extends Command {
  ...
  override def undoStep() : Unit = controller.game = controller.game.undoRedoMove(controller.game.cell(dy,dx), controller.game.cell(sy,sx))

  override def redoStep() : Unit = controller.game = controller.game.undoRedoMove(controller.game.cell(sy,sx), controller.game.cell(dy,dx))
}
