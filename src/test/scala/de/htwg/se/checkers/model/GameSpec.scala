...
class GameSpec extends AnyWordSpec with Matchers{
  ...
  "A Game" should {
    ...
    "be able to count kicked Pieces" in {
      losingPieces.countKickedPieces() should be(0,11)
    }
    "have a winner when opponent doesn't have pieces left" in {
      losingPieces.movePiece(losingPieces.cell(4,0), losingPieces.cell(6,2)) should be(lostPieces)
    }
    ...
    "have String representation if a winner exists" in {
      lostPieces.toString should be(lostString)
    }
  }
}
